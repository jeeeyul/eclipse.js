...
public class JavaAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	...
	private static IScanner fgScanner = ToolFactory.createScanner(false, false,
			false, false);
	/**
	 * The viewer.
	 * 
	 ...
	 *            default preferences
	 * @param viewer
	 *            the source viewer that this strategy is attached to
	 */
	public JavaAutoIndentStrategy(String partitioning,
			IJavaScriptProject project, ISourceViewer viewer) {
	...
	private static int computeAnonymousPosition(IDocument document, int offset,
		...
		boolean hasNewToken = looksLikeAnonymousClassDef(document,
				partitioning, scanner, pos);
		int openingParen = -1;
		while (true) {
	...
	private boolean isClosed(IDocument document, int offset, int length) {
		...
		ASTNode node = NodeFinder.perform(compilationUnit, relativeOffset,
				length);

		if (length == 0) {
			while (node != null
					&& (relativeOffset == node.getStartPosition() || relativeOffset == node
		...
		case ASTNode.IF_STATEMENT: {
			...
			if (elseStatement != null) {
				int sourceOffset = thenRegion.getOffset()
						+ thenRegion.getLength();
				int sourceLength = elseRegion.getOffset() - sourceOffset;
				IRegion elseToken = getToken(document, new Region(sourceOffset,
						sourceLength), ITerminalSymbols.TokenNameelse);
		...
		case ASTNode.FOR_STATEMENT: {
			Expression expression = node.getNodeType() == ASTNode.WHILE_STATEMENT ? ((WhileStatement) node)
					.getExpression() : ((ForStatement) node).getExpression();
			IRegion expressionRegion = createRegion(expression, info.delta);
			Statement body = node.getNodeType() == ASTNode.WHILE_STATEMENT ? ((WhileStatement) node)
					.getBody() : ((ForStatement) node).getBody();
	...
	private static void installJavaStuff(Document document) {
		String[] types = new String[] { IJavaScriptPartitions.JAVA_DOC,
		...
		FastPartitioner partitioner = new FastPartitioner(
				new FastJavaPartitionScanner(), types);
		partitioner.connect(document);
		document.setDocumentPartitioner(
				IJavaScriptPartitions.JAVA_PARTITIONING, partitioner);
	}
	...
	private static void removeJavaStuff(Document document) {
		document.setDocumentPartitioner(
				IJavaScriptPartitions.JAVA_PARTITIONING, null);
	}
	...
	private void smartPaste(IDocument document, DocumentCommand command) {
		...
		try {
			...
			for (int l = first; l < lines; l++) { // we don't change the number
													// of lines while adding
													// indents
	...
	private static String getCurrentIndent(Document document, int line)
		...
		if (to > from && to < endOffset - 1
			...
			String type = TextUtilities.getContentType(document,
					IJavaScriptPartitions.JAVA_PARTITIONING, to, true);
	...
	private void addIndent(Document document, int line, CharSequence indent,
		...
		if (newInsert > insert) {
			...
			if (whitespaceCount != 0
					&& whitespaceCount >= CodeFormatterUtil
							.getIndentWidth(fProject))
				insert = newInsert;
		}
	 ...
	 * @param token
	 *            the token
	 * @return the position after the scope or
	 *         <code>JavaHeuristicScanner.NOT_FOUND</code>
	 */
	private static int skipScope(JavaHeuristicScanner scanner, int start,
			int token) {
		int openToken = token;
		int closeToken;
		switch (token) {
	...
	private static IRegion createRegion(ASTNode node, int delta) {
		return node == null ? null : new Region(
				node.getStartPosition() + delta, node.getLength());
	}
